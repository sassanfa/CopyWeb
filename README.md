...
| `1.3.6` | تثبیت ذخیره زنده و شروع بدون پرش | Splash شفاف متحرک، حذف flicker، Title Bar فعال/غیرفعال، رفع Access Denied مرورگر، دریافت کامل‌تر WebP/AVIF از HTML، CSS، cache و service worker، ارسال Referer برای CDNها، ثبت ذخیره زنده در پروژه‌ها، نشان «● زنده»، پیش‌نمایش localhost و حذف امن پروژه از داشبورد |
...
- `CopyWeb-Windows-x64-1.3.6.zip` — نسخهٔ معمولی
- `CopyWeb-Portable-1.3.6.zip` — نسخهٔ قابل حمل و self-contained
- `CopyWeb-CLI-1.3.6.zip` — بستهٔ خط فرمان
- `CopyWeb-Setup-1.3.6.exe` — فایل نصب Windows
